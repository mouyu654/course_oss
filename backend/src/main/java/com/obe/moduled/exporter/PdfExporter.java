...
public class PdfExporter {
    ...
    public static byte[] generateCourseReport(String courseName, String className,
        ...
        try (PDDocument doc = new PDDocument();
            ...
            y = writeLine(cs, font, 11, MARGIN, y,
                    safeText("Course: " + courseName + "    Class: " + className, font));
            if (calcTime != null) {
                y = writeLine(cs, font, 10, MARGIN, y,
                        safeText("Calc Time: " + calcTime.format(DTF), font));
            }
            y -= LINE_HEIGHT;

            // Objective results
            y = writeLine(cs, font, 13, MARGIN, y,
                    safeText("--- Objective Achievements (Level 1) ---", font));
            for (Map.Entry<String, BigDecimal> e : objectiveResults.entrySet()) {
                y = writeLine(cs, font, 10, MARGIN + 20, y,
                        safeText("Objective " + e.getKey() + ": " + e.getValue(), font));
            }
            y -= LINE_HEIGHT;

            // Indicator results
            y = writeLine(cs, font, 13, MARGIN, y,
                    safeText("--- Indicator Achievements (Level 2) ---", font));
            for (Map.Entry<String, BigDecimal> e : indicatorResults.entrySet()) {
                y = writeLine(cs, font, 10, MARGIN + 20, y,
                        safeText("Indicator " + e.getKey() + ": " + e.getValue(), font));
            }
    ...
    /**
     * Strip characters that the font can't render. Chinese text in course names
     * and labels will be replaced with '?' when only ASCII fonts are available.
     */
    private static String safeText(String text, PDFont font) {
        if (text == null) return "";
        if (font == null) {
            // No font available: strip non-ASCII
    ...
    private static float writeLine(PDPageContentStream cs, PDFont font, float fontSize,
        ...
        if (font == null) return y; // skip rendering if no font available
        cs.beginText();
    ...
    private static PDFont loadFont(PDDocument doc) {
        if (cachedFont != null) {
            try { cachedFont.encode("test"); return cachedFont; } catch (Exception ignored) {}
        }
        // 1. Classpath fonts
        for (String path : new String[]{
            ...
            try {
                ...
                if (is != null) {
                    cachedFont = PDType0Font.load(doc, is);
                    return cachedFont;
                }
            } catch (Exception ignored) {}
        }
        ...
        for (String dir : new String[]{"C:/Windows/Fonts/", "C:/WinNT/Fonts/"}) {
            for (String name : new String[]{"simhei.ttf", "simsun.ttc", "msyh.ttc", "msyh.ttf"}) {
                try {
                    ...
                    if (f.exists()) {
                        cachedFont = PDType0Font.load(doc, f);
                        return cachedFont;
                    }
                } catch (Exception ignored) {}
            }
        }
        // 3. System fonts (Linux)
        for (String path : new String[]{
            ...
            try {
                if (Files.exists(Paths.get(path))) {
                    cachedFont = PDType0Font.load(doc, new File(path));
                    return cachedFont;
                }
            } catch (Exception ignored) {}
        }
        // 4. Fallback: PDFBox built-in Helvetica (ASCII only, safeText strips Chinese)
        return null;
    }
}
