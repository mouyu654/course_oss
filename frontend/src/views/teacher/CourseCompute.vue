    ...
    <el-card shadow="never">
      ...
      <div v-if="calcResult">
        ...
        <el-row :gutter="16" style="margin-bottom:20px">
          ...
          <el-col :span="6">
            <div class="stat-card">
              <div class="stat-number" :style="{ color: overallPassRate >= 60 ? '#67c23a' : '#f56c6c' }">
          ...
          <el-col :span="6">
            <div class="stat-card">
              <div class="stat-number" :style="{ color: passCount === indicatorTable.length ? '#67c23a' : '#e6a23c' }">
                {{ passCount }}/{{ indicatorTable.length }}
              </div>
            ...
            <el-card shadow="never" class="result-card">
              ...
              <div v-for="row in objectiveTable" :key="row.id" class="achievement-row">
                ...
                <el-progress
                  ...
                  :stroke-width="18"
                  :color="row.value >= 0.6 ? '#67c23a' : row.value >= 0.5 ? '#e6a23c' : '#f56c6c'"
        ...
        <el-card shadow="never" class="result-card" style="margin-top:20px">
          ...
          <el-table :data="indicatorTable" stripe size="small" style="width:100%">
            ...
            <el-table-column label="数值" width="100" align="center">
              <template #default="{ row }">
                <span :class="row.value >= 0.6 ? 'text-pass' : 'text-fail'" style="font-weight:600">
                  {{ row.value?.toFixed(4) }}
                </span>
              </template>
            </el-table-column>
            <el-table-column label="状态" width="90" align="center">
              <template #default="{ row }">
                <el-tag :type="row.value >= 0.6 ? 'success' : 'danger'" size="small" effect="dark">
        ...
        <p v-if="calcResult.calcTime" style="margin-top:20px;text-align:right;font-size:13px;color:#909399">
  ...
  chartInstance.setOption({
    tooltip: {
      ...
      formatter: p => {
        ...
        return `<b>${item.name}</b><br/>达成度：${item.value.toFixed(4)}（${pct}%）`
      }
    },
    ...
    series: [{
      ...
      data: data.map(d => ({
        value: d.value,
        itemStyle: {
          color: d.value >= 0.6 ? '#67c23a' : d.value >= 0.5 ? '#e6a23c' : '#f56c6c',
      ...
      markLine: {
        ...
        label: { fontSize: 11, formatter: '60%' },
        data: [{ yAxis: 0.6 }]
      }
    }]
  })
...
function downloadBlob(blob, filename, minSize = 200) {
  if (!blob || blob.size < minSize) {
    ElMessage.warning('文件内容为空，请确认已完成计算')
    return
  }
...
async function handleDownloadPdf() {
  ...
  try {
    const res = await downloadCoursePdf(selectedClassId.value)
    downloadBlob(res, '课程达成度报告.pdf', 200)
...
async function handleDownloadExcel() {
  ...
  try {
    const res = await downloadCourseExcel(selectedClassId.value)
    downloadBlob(res, '课程达成度报告.xlsx', 200)
